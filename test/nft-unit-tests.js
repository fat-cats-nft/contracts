...
describe("NFT Collectible", function () {
    ...
    it("Should reserve X NFTs", async function () {
        // Deploy the contract
        const Factory = await ethers.getContractFactory("NFTCollectible");
        const contract = await Factory.deploy(initialBaseTokenURI);
        await contract.deployed();
        ...
        try {
            await contract.reserveNFTs(1001);
        } catch (error) {
            expect(error.message).to.equal("VM Exception while processing transaction: reverted with reason string 'Not enough NFTs'");
        }
